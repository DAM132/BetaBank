...
public class MovPrestamoImp{
    ...
    public List<MovPrestamo> listar() {
     ...
     public ArrayList<MovPrestamo> listarPorCliente(int idPrestamo) {
        String sql = "SELECT * FROM movprestamo where idPrestamo=?";
        ArrayList<MovPrestamo> movimientosPrestamos = new ArrayList<>();
         try ( PreparedStatement stmt = getConnection().prepareStatement(sql);) {
            stmt.setInt(1, idPrestamo);
            try ( ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    MovPrestamo movimientoPrestamo = crearMovPrestamo(rs);
                    movimientosPrestamos.add(movimientoPrestamo);
                }
            }
